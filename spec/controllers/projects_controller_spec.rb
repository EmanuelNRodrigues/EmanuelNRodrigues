...
RSpec.describe(ProjectsController, type: :request) do
  ...
  describe 'show' do
    ...
    before { get user_project_path(user_id, project_id) }

    context 'when the pretended project exists' do
      let(:project_id) { project.id }
      let(:user_id) { user.id }
      ...
      it 'is expected to respond with the pretended project' do
        project.reload
        expect(project.attributes).to include(JSON(response.body).except('start_date', 'updated_at'))
      end
    end
  ...
  describe 'destroy' do
    let(:role) { 'admin' }
    let(:destroy_request) { delete user_project_path(user_id, project_id) }
    ...
    context 'when the pretended project exists' do
      ...
      it 'is expected to delete the pretended project' do
        expect { destroy_request }.to change(Project, :count).by(-1)
      end
    end
